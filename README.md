...
# Терминал
![](/images/lab_09/terminal.png)

# До запуска
![](/images/lab_09/before_start.png)

# После запуска
![](/images/lab_09/after_start.png)
...
print(f"\nПосле удаления Сидорова: {len(group.list())} студентов")
```
# students.csv после обновления и удаления
![](/images/lab_09/deleted_and_updated.png)


# Лабораторная работа 10 (Наконец-то)
# Теория - Структуры данных: Стек, Очередь, Односвязный список
...
- Вызов функций в программах (стек вызовов)
...
- Удаление с начала `pop(0)` — O(n) (все элементы сдвигаются)

**`collections.deque` — оптимальный выбор:**
- `append()` / `appendleft()` — O(1)
- `pop()` / `popleft()` — O(1)

### Асимптотика (на корректной реализации):
- `enqueue` — O(1)
- `dequeue` — O(1)
- `peek` — O(1)
...
### Структура:
Состоит из узлов `Node`, где каждый узел хранит:
- `value` — значение элемента
- `next` — ссылка на следующий узел или `None` (если это последний)
...
- Удобно использовать как базовый строительный блок для других структур (очереди, стеки, хеш-таблицы с цепочками)
- Динамическое изменение размера (не требуется заранее выделять память)

### Недостатки:
- Доступ по индексу i — O(n) (нужно пройти i шагов от головы)
- Нет быстрого доступа к предыдущему элементу
- Больше используемой памяти (на хранение ссылок)
- Кэш-промахи из-за нелокальности данных в памяти

### Типичные временные оценки:
- `prepend` (добавить в начало) — O(1)
- `append` (добавить в конец):
  - При наличии `tail` — O(1)
  - Без `tail` — O(n) (требуется пройти до конца)
- Поиск по значению — O(n)
- Удаление элемента (при известной позиции) — O(1)
- Поиск позиции элемента — O(n)

### Базовые операции для работы со списком:
